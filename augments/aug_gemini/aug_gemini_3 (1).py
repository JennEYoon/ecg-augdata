...
Original file is located at
    https://colab.research.google.com/drive/1EC48Y-xdF_w8pc0nNuW-tjW6gxokryDi
...
-- 00:15 am, finished V3 class, combine triple Vs with original ECGAugmenter class  
 -- 2:00 am, finished all augments, tested  
 -- 2L20 am stopped, download final nb.
...
"""### 2: define class ECGAugmenterV3  
This uese triple level chest leads combination, developed below bb
in class PrecordialAugmenter  
"""
...
"""### 3: Load Example Files, define ecg_data
define dirin, dirout  
input file defines ecg_data  
*.mat format  
"""
...
"""### 4: Run, Initialize Augmenter  
calls class ECGAugmenterV3  
ecg_date is defined from input file and dir
plot lead II as example  

 * later, save plot of all 12 leads, orig vs aug
"""
...
def sample_plots(LEAD_TO_PLOT, orignial_ecg, augmented_ecg_np):
    try:

        lead_to_plot = LEAD_TO_PLOT
        t = np.arange(original_ecg.shape[1]) / SAMPLING_RATE
        ...
        figname = 'ecg_aug_orig_' + lead_to_plot + '.png'
        plt.savefig(figname)
        plt.show()
...
for lead in lead_list:
    sample_plots(lead, original_ecg, augmented_ecg_np)
...
for lead_name in precordial_leads_to_plot:
    ...
    figname = 'ecg_chest_' + lead_name + '_aug.png'
    plt.savefig(figname)
...
### Finished Augmenter class  
 Complete augments work well  
 Powerline tested, change interval to short  
 Sharp spike up, baseline large drop seen.  
 Muscle and noise also seem to work, have seen.  

 All in all, a good set of robust noise.

### 6: Todo: Run in batch mode, from Terminal  
Refactor code to run in batch mode  
os.walk to read each file in dir = "g1_dwmv", create filename_list  

Then for each item in list, initiate class, run ECGAugmenterV3, output processed file to dir "g1_aug". Skip plotting.  
